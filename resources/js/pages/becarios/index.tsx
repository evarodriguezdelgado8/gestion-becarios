...
const initials = (becario: Intern) =>
    `${becario.name?.charAt(0) ?? ''}${becario.last_name?.charAt(0) ?? ''}`.toUpperCase();
        ...
        <Popover>
            ...
            <PopoverContent className="w-auto p-0" align="start">
                <Calendar
                    ...
                    onSelect={(date) =>
                        onChange(name, date ? format(date, 'yyyy-MM-dd') : '')
                    }
                />
            </PopoverContent>
        </Popover>
...
export default function Index({ interns, filters, centers, flash }: Props) {
    ...
    const isTutor = auth.user?.roles?.some((r: any) => {
        const roleName = typeof r === 'object' ? r.name : r;
    ...
    const [invitingInternId, setInvitingInternId] = useState<number | null>(
        null,
    );
    ...
    const [selectedCenters, setSelectedCenters] = useState<string[]>(
        filters.center_id ? filters.center_id.split(',').filter(Boolean) : [],
    );
    const activeFiltersCount = Object.values(params).filter(Boolean).length;
    const visibleInternIds = useMemo(
        () => interns.data.map((becario) => becario.id),
        [interns.data],
    );
    const selectedVisibleCount = visibleInternIds.filter((id) =>
        selectedInternIds.includes(id),
    ).length;
    const isHeaderSelectionChecked =
        allFilteredSelected ||
        (visibleInternIds.length > 0 &&
            selectedInternIds.length > 0 &&
            interns.total === selectedInternIds.length &&
            selectedVisibleCount === visibleInternIds.length);
    const isHeaderSelectionIndeterminate =
        !allFilteredSelected && selectedInternIds.length > 0;
    const bulkInvitationCount =
        invitationMode === 'selected' && !allFilteredSelected
            ? selectedInternIds.length
            : interns.total;
    const isInvitingAllFiltered =
        invitationMode === 'filtered' ||
        (invitationMode === 'selected' && allFilteredSelected);
    ...
    */
    ...
    const sendInvitation = (becario: Intern) => {
        setInvitingInternId(becario.id);

        router.post(
            `/becarios/${becario.id}/invitar`,
            {},
            {
                preserveScroll: true,
                onFinish: () => setInvitingInternId(null),
            },
        );
    };
    ...
    const toggleInternSelection = (internId: number) => {
        if (allFilteredSelected) {
            setAllFilteredSelected(false);
            setSelectedInternIds(
                visibleInternIds.filter((id) => id !== internId),
            );
    ...
    const toggleAllFilteredSelection = () => {
        setAllFilteredSelected((current) => !current);
        setSelectedInternIds([]);
    };

    const sendBulkInvitations = () => {
        if (!invitationMode || bulkInvitationCount === 0) return;

        setIsBulkInviting(true);

        router.post(
            '/becarios/invitaciones',
            {
                mode:
                    invitationMode === 'selected' && !allFilteredSelected
                        ? 'selected'
            ...
            },
            {
                ...
                onFinish: () => setIsBulkInviting(false),
            },
        );
    };
            ...
            ? [
                  {
                      header: (
                          <div className="flex justify-center">
                              <Checkbox
                                  checked={
                                      isHeaderSelectionChecked
                                          ? true
                                          : isHeaderSelectionIndeterminate
                                            ? 'indeterminate'
                                            : false
                                  }
                                  onCheckedChange={toggleAllFilteredSelection}
                                  aria-label="Seleccionar todos los becarios del listado"
                              />
                          </div>
                      ),
                      className: 'w-[56px]',
                      cellClassName: 'align-middle',
                      render: (becario: Intern) => (
                          <div className="flex justify-center">
                              <Checkbox
                                  checked={
                                      allFilteredSelected ||
                                      selectedInternIds.includes(becario.id)
                                  }
                                  onCheckedChange={() =>
                                      toggleInternSelection(becario.id)
                                  }
                                  aria-label={`Seleccionar ${becario.name} ${becario.last_name}`}
                              />
                          </div>
                      ),
                  },
            ...
            : []),
        {
            ...
            render: (becario) => (
                <div className="flex items-center gap-3">
                    <Avatar className="h-11 w-11 rounded-2xl ring-1 ring-blue-100">
                        <AvatarImage
                            src={becario.user?.photo_url || undefined}
                            alt={`${becario.name} ${becario.last_name}`}
                            className="object-cover"
                        />
                        <AvatarFallback className="rounded-2xl bg-blue-50 text-sm font-bold text-blue-600">
                            {initials(becario) || <User className="h-5 w-5" />}
                        </AvatarFallback>
                    </Avatar>
                    <div className="space-y-1">
                        <Link
                            href={`/becarios/${becario.id}`}
                            className="text-base leading-tight font-bold text-slate-900 transition-colors hover:text-blue-600"
                        >
        ...
        },
        {
            ...
            render: (becario) => (
                <div className="space-y-2">
                    <div className="flex items-start gap-1.5 text-base leading-tight font-bold text-slate-900">
                        <Building2 className="mt-1 h-4 w-4 flex-shrink-0 text-slate-400" />
                        {becario.center ? (
                            <Link
                                href={`/centros/${becario.center.id}`}
                                className="break-words transition-colors hover:text-blue-600"
                            >
                                {becario.center.name}
                            </Link>
                        ) : (
        ...
        },
        {
            ...
            render: (becario) => {
                ...
                const progress = Math.round(
                    (becario.completed_hours / (becario.total_hours || 400)) *
                        100,
                );
                return (
                    <div className="space-y-3">
                        <div className="flex items-end justify-between text-[11px]">
                            <span
                                className={`rounded-full border px-2.5 py-1 font-black uppercase ${statusInfo.class}`}
                            >
                                {statusInfo.label}
                            </span>
                        ...
                        <div className="flex items-center gap-1 text-xs font-semibold text-slate-500">
                            ...
                            {formatDateDisplay(becario.start_date)} -{' '}
                            {formatDateDisplay(becario.end_date)}
                        </div>
                    </div>
                );
            },
        ...
        <AppLayout breadcrumbs={[{ title: 'Becarios', href: '/becarios' }]}>
                ...
                <div className="flex flex-col items-center justify-between gap-4 md:flex-row">
                    ...
                    <div className="flex flex-wrap justify-end gap-3">
                        {canInvite && (
                            <>
                                ...
                                >
                                    ...
                                    {(allFilteredSelected ||
                                        selectedInternIds.length > 0) &&
                                        ` (${allFilteredSelected ? interns.total : selectedInternIds.length})`}
                                </Button>
                                <Button
                                    type="button"
                                    variant="outline"
                                    onClick={() =>
                                        setInvitationMode('filtered')
                                    }
                                    disabled={interns.total === 0}
                                    className="flex items-center gap-2 rounded-xl border-emerald-200 px-4 py-2 text-sm font-bold text-emerald-700 shadow-sm transition hover:bg-emerald-50 disabled:opacity-50"
                                >
                                    <Send className="h-4 w-4" />
                                    Invitar listado ({interns.total})
                                </Button>
                            </>
                        )}
                        <a
                            href={`/becarios/export?${new URLSearchParams(params as any)}`}
                        ...
                        {isAdmin && (
                            ...
                            >
                                <Plus className="h-4 w-4" /> Nuevo Becario
                            </Link>
                        )}
                    </div>
                </div>
                    ...
                    {filtersOpen && (
                        <div className="space-y-4 border-t border-slate-100 px-5 pt-4 pb-5">
                            ...
                            <div className="grid grid-cols-1 gap-4 border-t border-slate-100 pt-4 sm:grid-cols-2 lg:grid-cols-5">
                                ...
                                <FilterDatePicker
                                    name="start_from"
                                    value={params.start_from}
                                    label="Inicio (Desde)"
                                    onChange={handleFilterChange}
                                />
                                <FilterDatePicker
                                    name="start_to"
                                    value={params.start_to}
                                    label="Inicio (Hasta)"
                                    onChange={handleFilterChange}
                                />
                                <FilterDatePicker
                                    name="end_from"
                                    value={params.end_from}
                                    label="Fin (Desde)"
                                    onChange={handleFilterChange}
                                />
                                <FilterDatePicker
                                ...
                                <div className="flex flex-col">
                                    <label className="mb-1 block text-[10px] font-bold uppercase opacity-0 select-none">
                                        Espaciador
                                    </label>
                                    <ClearFiltersButton
                ...
                <DataTable
                    ...
                    emptyMessage={
                        <div className="flex flex-col items-center gap-2 py-6">
                            ...
                            <span>
                                No se encontraron becarios con estos filtros.
                            </span>
                        </div>
                    }
                    actions={(becario) => (
                        <div className="flex justify-end gap-1.5">
                            <Link
                                href={`/becarios/${becario.id}`}
                                className="rounded-xl border border-slate-200 bg-white p-2 text-slate-500 shadow-sm transition-all hover:bg-slate-50 hover:text-slate-900"
                                title="Ver"
                            >
                            ...
                            {canInvite && (
                                <button
                                    type="button"
                                    onClick={() => sendInvitation(becario)}
                                    disabled={invitingInternId === becario.id}
                                    className="cursor-pointer rounded-xl border border-slate-200 bg-white p-2 text-slate-500 shadow-sm transition-all hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
                            ...
                            {isAdmin && (
                                <>
                                    <Link
                                        ...
                                        className="rounded-xl bg-blue-600 p-2 text-white shadow-sm transition-all hover:bg-blue-700"
                                    >
            ...
            {invitationMode && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-md animate-in overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-2xl duration-200 fade-in zoom-in">
                        <div className="p-6 text-center">
                            ...
                            <p className="mt-2 text-sm text-gray-500">
                                {!isInvitingAllFiltered
                                    ? `Se enviaran invitaciones a ${bulkInvitationCount} becario(s) seleccionado(s).`
                                    : `Se enviaran invitaciones a los ${bulkInvitationCount} becario(s) del listado filtrado.`}
                            </p>
                            {isInvitingAllFiltered &&
                                activeFiltersCount === 0 && (
                                    <p className="mt-3 rounded-xl bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">
                                        No hay filtros activos, asi que se
                                        invitara a todos los becarios que puedes
                                        gestionar.
                                    </p>
                                )}
                        </div>
                        ...
                        <div className="flex gap-3 bg-gray-50 px-6 py-4">
                            <button
                                type="button"
                                onClick={() => setInvitationMode(null)}
                                disabled={isBulkInviting}
                                className="flex-1 cursor-pointer rounded-xl border border-gray-300 bg-white px-4 py-2.5 font-bold text-gray-700 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                                Cancelar
                            </button>
                            <button
                                ...
                                disabled={
                                    isBulkInviting || bulkInvitationCount === 0
                                }
                                className="flex-1 cursor-pointer rounded-xl bg-emerald-600 px-4 py-2.5 font-bold text-white shadow-lg shadow-emerald-200 transition-colors hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                                {isBulkInviting ? 'Enviando...' : 'Si, enviar'}
                            </button>
                        </div>
                    </div>
                </div>
