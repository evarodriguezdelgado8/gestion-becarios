        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xl">
            <div className="overflow-x-auto">
                <Table className="min-w-[900px]">
                    <TableHeader className="border-b border-slate-100 bg-slate-50/80 text-[11px] font-black tracking-widest text-slate-400 uppercase">
                        <TableRow className="border-slate-100 hover:bg-transparent">
                            {columns.map((col, i) => (
                                <TableHead
                                    key={i}
                                    className={`px-6 py-4 font-black text-slate-400 ${col.className ?? ''}`}
                                >
                                    {col.header}
                                </TableHead>
                            ))}
                            {actions && (
                                <TableHead className="w-[128px] px-6 py-4 text-right font-black text-slate-400"></TableHead>
                            )}
                        </TableRow>
                    </TableHeader>
                            ...
                            data.map((item, rowIndex) => (
                                ...
                                >
                                    {columns.map((col, colIndex) => (
                                        <TableCell
                                            key={colIndex}
                                            className={`px-6 py-5 align-top whitespace-normal ${col.cellClassName ?? ''}`}
                                        >
                                            {col.render(item)}
                                        </TableCell>
                                    ))}
                                    {actions && (
                                        <TableCell className="px-6 py-5 text-right align-middle whitespace-normal">
                                            {actions(item)}
                                        </TableCell>
                                    )}
                                </TableRow>
